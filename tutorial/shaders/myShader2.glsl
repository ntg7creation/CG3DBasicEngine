...
void main() {
    ...
    vec4 new_eye = z_rotation*y_rotation*x_rotation*mtranslate * vec4(eye.xyz,1);//y_rotation *x_rotation* eye;
   
	vec3 normalizedRayDirection = normalize(new_pos.xyz - new_eye.xyz); // TODO : figure out why it's negative
	...
	for (int objectIndex = 0; objectIndex < numOfObjects; ++objectIndex) {
		res = rayObjectIntersection(new_eye.xyz, normalizedRayDirection, objectIndex);
